(function () {
  ...
  main.innerHTML = `
    <section class="hero section-shell" id="about">
      ...
      <aside class="profile-panel reveal" aria-label="Personal Overview"><div class="portrait-wrap"><img src="${escapeHTML(p.avatar)}" alt="${escapeHTML(p.name)}'s Portrait" /><span class="status-dot" aria-hidden="true"></span></div><div class="profile-affiliation">${p.affiliation ? (p.affiliation.includes("·") ? p.affiliation.split("·").map((s) => `<span>${escapeHTML(s.trim())}</span>`).join("") : `<span>${escapeHTML(p.affiliation)}</span>`) : ""}</div>${facts ? `<div class="facts-grid">${facts}</div>` : ""}</aside>
    </section>
